...
var _Sources = (() => {
  ...
  // src/RaijinScan/crypto-js.min.js
  var require_crypto_js_min = __commonJS({
    "src/RaijinScan/crypto-js.min.js"(exports, module) {
      "use strict";
      !function(t, e) {
  ...
  __export(RaijinScan_exports, {
    RaijinScan: () => RaijinScan,
    RaijinScanInfo: () => RaijinScanInfo
  });
  var import_types3 = __toESM(require_lib());

  // src/RaijinScan/Madara.ts
  ...
  // src/RaijinScan/MadaraDecrypter.ts
  ...
  // src/RaijinScan/MadaraParser.ts
  var Parser3 = class {
    constructor() {
      this.parseDate = (date) => {
  ...
  // src/RaijinScan/MadaraHelper.ts
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  var RaijinScanParser = class extends Parser3 {
    async parseMangaDetails($2, mangaId, source) {
      ...
      const author = decode($2('span.stat-label:contains("Auteur")').next().text().replace("\\n", "").trim()).replace("Updating", "");
      const artist = decode($2('span.stat-label:contains("Artiste")').next().text().replace("\\n", "").trim()).replace("Updating", "");
      const description = decode($2("div.description-content").text()).replace("Show more", "").trim();
      const image = encodeURI(await this.getImageSrc($2("img.cover"), source));
      const parsedStatus = $2('span.stat-label:contains("\xC9tat du titre")').next().text().trim();
      let status;
      switch (parsedStatus) {
        case "Termin\xE9":
          status = "Completed";
      ...
      const genres = [];
      for (const obj of $2("div.genres-link").toArray()) {
        const label = $2(obj).text();
        const id = $2(obj).attr("href")?.split("/")[4] ?? label;
        if (!label || !id) continue;
        genres.push(App.createTag({ label, id }));
  ...
  var RaijinScan = class extends Madara {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
      ...
      this.parser = new RaijinScanParser();
