...
var _Sources = (() => {
      ...
      (function(global, factory) {
        ...
        function localeMonthsParse(monthName, format2, strict2) {
          ...
          for (i = 0; i < 12; i++) {
            ...
            if (strict2 && format2 === "MMMM" && this._longMonthsParse[i].test(monthName)) {
              return i;
            } else if (strict2 && format2 === "MMM" && this._shortMonthsParse[i].test(monthName)) {
              return i;
        ...
        function localeWeekdaysParse(weekdayName, format2, strict2) {
          ...
          for (i = 0; i < 7; i++) {
            ...
            if (strict2 && format2 === "dddd" && this._fullWeekdaysParse[i].test(weekdayName)) {
              return i;
            } else if (strict2 && format2 === "ddd" && this._shortWeekdaysParse[i].test(weekdayName)) {
              return i;
            } else if (strict2 && format2 === "dd" && this._minWeekdaysParse[i].test(weekdayName)) {
              return i;
        ...
        function isMomentInput(input) {
          return isMoment(input) || isDate(input) || isString(input) || isNumber4(input) || isNumberOrStringArray(input) || isMomentInputObject(input) || input === null || input === void 0;
        }
        ...
        function isNumberOrStringArray(input) {
          ...
          if (arrayTest) {
            dataTypeTest = input.filter(function(item) {
              return !isNumber4(item) && isString(input);
            }).length === 0;
          }
  ...
  var PhenixScans_exports = {};
  __export(PhenixScans_exports, {
    PhenixScans: () => PhenixScans,
    PhenixScansInfo: () => PhenixScansInfo
  });
  ...
  // src/PhenixScans/PhenixScansHelpers.ts
  ...
  function convertDate(dateString) {
    ...
    return (0, import_moment.default)().subtract(amount, mappedUnit).toDate();
  }

  // src/PhenixScans/PhenixScansParser.ts
  var PhenixScansParser = class {
    parseMangaDetails($2, mangaId, source) {
      const titles = [];
      titles.push(decode($2("h1.project__content-informations-title").text().trim()));
      const author = "";
      const artist = "";
      const image = this.getImageSrc($2("img.project__cover"));
      const description = decode($2("div.project__content-synopsis > p").text().trim());
      const arrayTags = [];
      for (const tag of $2("a", "project__content-tags").toArray()) {
    ...
    parseChapterList($2, mangaId, source) {
      ...
      for (const chapter of $2("a.project__chapter").toArray()) {
        const title = decode($2("span.project__chapter-title", chapter).text().trim()).replace(/\s+/g, " ");
        const date = convertDate($2("div.project__chapter-date", chapter).text().trim());
    ...
    parseChapterDetails($2, mangaId, chapterId) {
      const pages = [];
      for (const img of $2("div.vertical.chapter-images > img").toArray()) {
        pages.push(this.getImageSrc($2(img)));
      }
    ...
    parseTags($2, genres) {
      ...
      const dropdowns = $2("div.manga-list__filter-group").toArray();
      for (let i = 1; i < 4; ++i) {
        const sectionDropdown = dropdowns[i];
        if (!sectionDropdown) {
          continue;
        }
        for (const tag of $2("option", sectionDropdown).toArray()) {
          const label = $2(tag).attr("value")?.trim();
    ...
    async parseHomeSection($2, section, source) {
      ...
      for (const manga of mangas.toArray()) {
        ...
        const image = this.getImageSrc($2("[class$=card-image] img", manga)) ?? "";
        const subtitle = section.subtitleSelectorFunc($2, manga) ?? "";
        let slug = this.idCleaner($2("a", manga).attr("href") ?? "");
        if (slug === "") {
          slug = this.idCleaner($2(manga).attr("href") ?? "");
        }
        const mangaId = slug;
        if (!mangaId || !title) {
  ...
  var PhenixScans = class {
    constructor() {
      ...
      this.parser = new PhenixScansParser();
      this.homescreen_sections = {
        "our_projects": {
          ...DefaultHomeSectionData,
          section: createHomeSection("our_projects", "Nos projets", false, import_types3.HomeSectionType.featured),
          selectorFunc: ($2) => $2("a.main__our-project-card", $2("div.main__our-project-list")),
        ...
        "latest_update": {
          ...
          selectorFunc: ($2) => $2("div.main__latest-releases-card", $2("div.main__latest-releases-list")),
          titleSelectorFunc: ($2, element) => $2("h3", element).text(),
          subtitleSelectorFunc: ($2, element) => void 0,
          getViewMoreItemsFunc: (page) => void 0,
          sortIndex: 20
        ...
        "new_titles": {
          ...DefaultHomeSectionData,
          section: createHomeSection("new_titles", "Les nouvelles s\xE9ries"),
          selectorFunc: ($2) => $2("div.embla__slide", $2("div.main__new-releases")),
          titleSelectorFunc: ($2, element) => $2("h3", element).text(),
          subtitleSelectorFunc: ($2, element) => void 0,
          getViewMoreItemsFunc: (page) => void 0,
          sortIndex: 30
    ...
    async getSearchResults(query, metadata) {
      ...
      metadata = !this.parser.isLastPage(response.data) ? { page: page + 1 } : void 0;
    ...
    async getSearchTags() {
      ...
      this.checkResponseError(response);
      return this.parser.parseTags($2, response.data ?? "");
    }
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
    ...
    async constructSearchRequest(page, query) {
      let urlBuilder = new URLBuilder(this.baseUrl).addPathComponent("api").addPathComponent("front").addPathComponent(this.directoryPath);
      urlBuilder = urlBuilder.addQueryParameter("query", query?.title ?? "").addQueryParameter("page", page).addQueryParameter("limit", "18").addQueryParameter("genre", getFilterTagsBySection("genres", query?.includedTags, true)).addQueryParameter("status", getIncludedTagBySection("status", query?.includedTags)).addQueryParameter("type", getIncludedTagBySection("type", query?.includedTags)).addQueryParameter("sort", getIncludedTagBySection("sort", query?.includedTags));
  ...
  return __toCommonJS(PhenixScans_exports);
})();
