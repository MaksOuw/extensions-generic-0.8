...
var _Sources = (() => {
  ...
  var StarboundScans_exports = {};
  __export(StarboundScans_exports, {
    StarboundScans: () => StarboundScans,
    StarboundScansInfo: () => StarboundScansInfo
  });
  ...
  function convertDate(dateString) {
    ...
    if (match) {
      ...
      switch (unit) {
        case "second":
          dateString = now.getTime() - value * 1e3;
        case "minute":
          dateString = now.getTime() - value * 60 * 1e3;
        case "hour":
          dateString = now.getTime() - value * 60 * 60 * 1e3;
        case "day":
          dateString = now.getTime() - value * 24 * 60 * 60 * 1e3;
      }
    }
  ...
  var MangaStreamParser = class {
    ...
    parseChapterList($2, mangaId, source) {
      ...
      for (const chapter of $2("li", "div#chapterlist").toArray()) {
        const title = decode($2("span.chapternum", chapter).text().trim()).replace(/\s+/g, " ");
  ...
  // src/StarboundScans/StarboundScansParser.ts
  var StarboundScansParser = class extends MangaStreamParser {
    constructor() {
      super(...arguments);
      this.isLastPage = ($2, id) => {
        let isLast = true;
    ...
    parseChapterList($2, mangaId, source) {
      ...
      for (const chapter of $2("div#chapters > a").toArray()) {
        ...
        const id = decode($2(chapter).attr("href").trim()).substr(21, 11) ?? "";
        const chapterNumber = parseFloat(decode($2(chapter).attr("title").trim()).replace(/Chapitre\s/g, "") ?? "");
        if (!id || typeof id === "undefined") {
          throw new Error(`Could not parse out ID when getting chapters for postId: ${mangaId}`);
        }
        chapters.push({
    ...
    parseChapterDetails($2, mangaId, chapterId) {
      const pages = [];
      for (const img of $2("div#pages > img").toArray()) {
        const uid = $2(img).attr("uid") ?? "";
        if (!uid) continue;
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  var StarboundScans = class extends MangaStream {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
